...
## DecisÃµes fixas do app `yclf_dash_360_meta_compromisso`

Ao trabalhar neste projeto SAPUI5 Freestyle, preservar as decisÃµes abaixo. NÃ£o alterar sem solicitaÃ§Ã£o explÃ­cita do usuÃ¡rio.

### Layout e estilo

- Usar classes standard SAP Fiori/UI5 e propriedades XML dos controles.
- NÃ£o criar CSS novo para resolver layout, espaÃ§amento, cores ou comportamento visual.
- NÃ£o adicionar classes customizadas novas para estilo quando houver classe/propriedade SAPUI5 equivalente.
- NÃ£o recriar estilos de planilha, cores fortes ou overrides globais para tabelas/Object Pages.
- O app usa SAP Horizon e SAPUI5 `1.149.1`.

### Dashboard

- O FilterBar deve manter Smart Variant com nome `PadrÃ£o`.
- O FilterBar deve manter controles nativos de pin/collapse do `DynamicPageHeader`.
- O filtro `Consultor` deve usar `MultiComboBox`, com checkbox por item e `showSelectAll="true"`.
- A lista de consultores nÃ£o deve ter item mockado `Todos os escritÃ³rios`; usar apenas o `Select All` nativo do `MultiComboBox`.
- O relatÃ³rio `Acompanhamento diÃ¡rio` deve separar os segmentos em grupos `GNP` e `GNI`.
- Os textos `GNP/GNI` nÃ£o devem aparecer duplicados dentro das linhas.
- A coluna `Carteira` deve aparecer antes de `Carteira + Faturado`.
- No relatÃ³rio `Meta por quantidade de FamÃ­lia - Varejo`, o tÃ­tulo deve ser `Carteira + Faturado`.
- Os nomes de Segmento e FamÃ­lia devem permanecer em cor padrÃ£o, sem link azul.
- No relatório `Acompanhamento diário`, a navegação para Object Page deve acontecer somente pelo botão/ícone `sap-icon://navigation-right-arrow` na última coluna.
- No relatório `Meta por quantidade de Família - Varejo`, a linha da família deve usar `ColumnListItem type="Navigation"` e abrir a Object Page de Família; o nome da família deve permanecer sem link azul.

### Cores de colunas e valores

- Títulos e valores de `Compromisso` devem ficar em azul usando estado padrão SAPUI5 `Information`.
- Títulos e valores de `Faturado`/`Faturamento` devem permanecer na cor padrão SAPUI5, sem estado de destaque.
- Não criar CSS novo para essas cores; preferir `ObjectStatus` e `ObjectNumber state`.

### Object Pages

- Usar layout padrÃ£o SAPUI5 com `sap.uxap:ObjectPageLayout`.
- NÃ£o usar visual de planilha nem cores amarelas.
- Exibir o relatÃ³rio `CrÃ­ticas dos pedidos` somente nas Object Pages, usando tabela SAPUI5 padrÃ£o e dados mockados em `ReportService`.
- O relatório `Críticas dos pedidos` deve ter somente as colunas `Críticas`, `QTD`, `Valor` e `Porcentagem`.
- Nas colunas das Object Pages, nÃ£o exibir mÃªs em `OrÃ§amento` e `Compromisso`.
- A Object Page de Segmento nÃ£o deve exibir a coluna `Carteira mÃªs anterior`.
- A linha total da Object Page de Segmento deve aparecer como `Total`, nÃ£o como `ERIKA`.
